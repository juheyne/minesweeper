...
class Model:
    def __init__(self, size_y, size_x, num_actions, batch_size):
        self._size_y = size_y
        self._size_x = size_x
        self._dimensions = (size_y, size_x)  # (size_y, size_x, 2)
    ...
    def _define_model(self):
        ...
        fc1 = tf.layers.dense(self._input, 50, activation=tf.nn.relu)
        do1 = tf.layers.dropout(fc1, self._keep_prob)
        fc2 = tf.layers.dense(do1, 50, activation=tf.nn.relu)
